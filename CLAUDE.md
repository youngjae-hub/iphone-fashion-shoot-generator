...
| # | 과제 | 설명 |
|---|------|------|
| 2-1 | **ControlNet + OpenPose 포즈 제어** | 스켈레톤 기반 정확한 포즈 생성 (뒷면 포즈 해결, 기괴한 자세 방지) |
| 2-2 | A/B 비교 | 같은 입력으로 Provider 2개 결과를 나란히 비교 |
| 2-3 | 시드 고정 재현성 | 마음에 든 결과를 시드로 저장 → 같은 스타일로 다른 옷 생성 |
| 2-4 | 품질 자동 체크 | 생성 결과에 얼굴 노출 여부, 의류 반영도 자동 검증 |
